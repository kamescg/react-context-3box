...
// Require 3Box if the Window variable is set.
let box;
if (typeof window !== `undefined`) box = require('3box/dist/3box')
...
class BoxProvider extends React.Component {
  constructor(props){
    ...
    this.state  = {
      ...
      confirmRequest: async (index) => {
        ...
        if(req && req.payload) {        
          switch (req.type) {
            ...
            default:
                this.state.isDebugging && toast('No Request Type Set');
              break;
          }
      ...
      // Write
      /* ---------------------- */
      get: async ({ key, space, access, all }) => {
        let value
        try {
          if(space) {
            all 
       ...
       * @description 
       */
      removeItem: async ({index, space, access, list}) => {
        if(space) {
          const data = await this.state.spaces[space].instance[access].get(list)
          if (data && Array.isArray(data)) {
